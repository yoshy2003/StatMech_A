...
- 計算機上で完全なランダム性を実現することは困難であるため、線形合同法やメルセンヌツイスター法といった擬似乱数として知られるアルゴリズムに基づいた数値が使用される。
- 乱数は等確率に生成される（一様乱数の場合）だけでなく、特定の確率分布に従うように生成される場合もある。
  - 例えば、1次元ランダムウォークの結果として得られる位置を再現したい場合には、二項分布または正規分布に従った乱数を生成することになる。
  - このように、特定の確率分布に従う乱数を生成することで、物理現象の数値的模擬が可能となる。
...
- **コイン投げの系と確率分布**: $N$個のコインからなる系では、各コインは裏か表の2通りの状態を取り得るため、系全体の状態は $2^N$通り存在する。 $N$枚のコインのうち $X$枚が表である状態の数は、組合せの数式を用いて計算でき、この分布は二項分布になる。
- **確率変数と確率分布のタイプ**: 確率分布は離散的または連続的に表される。例えば、コインの状態は離散的（2状態）、粒子の位置は連続的に分布する。
- **確率密度の導入**: 連続変数における確率を表す際は、確率密度 $p(x)$を用いて、区間 $x$から $x+dx$までの確率が $p(x)dx$で与えられる。離散的な確率を連続的な確率密度で表す場合、デルタ関数と離散的な確率 $P_j$の和を用いる。
- **離散から連続への変換**: この変換では確率の総和が1であることが重要。連続表現では積分の和が1になるように確率が定義され、離散表現では和が1になる。
- **状態数密度の使用**: 近似として、状態数密度 $\rho$を用いて確率密度を表すことがあり、この表現は統計力学で頻繁に用いられる。状態数密度は微小区間内のデルタ関数の和で表され、その区間内に存在する状態の数を密度として表現する。ここでは、MATLABを用いて、２項分布と正規分布を学ぶ
...
モンテカルロ法は、乱数を利用した数値計算の一種であり、ここでは、乱数を利用して、特定の領域に対する面積を求める方法を紹介する。
例えば、正方形の領域内に特定の領域が存在するとして、正方形の面積を $S$、特定の領域の面積を $S_1$とする。
ここでは、 $S$は既知として、 $S_1$を求めることが目的となる。
実際、この計算は以下の次のステップで進めることができる。
...
このモンテカルロ法による面積計算は、特に複雑な形状の領域や解析的に積分が困難な場合に有効である。
